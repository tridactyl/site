...
for f in content/newsletters/*.md
    ...
    set creationtime (git log --format=%aI (basename $f) | tail -1)
    cd -
...
# for f in *.md; cd ../../../../tridactyl/doc/newsletters/tips-and-tricks/; set creationtime (git log --format=%aI (basename $f) | tail -1); cd -;  sed -i 's|^\# Tridactyl \(.*\)|---\ntitle: "\1"\ndate: "'$creationtime'"\ndraft: false\n---\n|g' $f; end
